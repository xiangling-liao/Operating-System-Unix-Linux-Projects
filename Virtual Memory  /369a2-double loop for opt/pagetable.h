...
#define PGTBL_INDEX(x)   (((x) >> PAGE_SHIFT) & PGTBL_MASK)  //shift right 20, and take low 10 bits
...
struct frame {
	...
	int distance; //!!! add it myself, to indicate how long time a page haven't been referred ?? need initialize?
	addr_t vaddr;
};
...
typedef struct tracenode{
		...
		addr_t vaddr;
} tracenode_t;    //!!!Added by myself: for opt algo to store the refered page

extern tracenode_t *tracelist_head;
extern tracenode_t *curr;
...
extern pgdir_entry_t init_second_level(); //!!! Added by myself
