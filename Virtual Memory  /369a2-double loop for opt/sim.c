...
char *tracefile = NULL;
node_t *referlist_head=NULL;
tracenode_t *curr=NULL;
tracenode_t *tracelist_head=NULL;
...
void access_mem(char type, addr_t vaddr) {
	...
	addr_t *checkaddr = (addr_t *)(memptr + sizeof(int));  //We then check that the memory has the expected content (just a copy of the virtual address) 
                                                           //addr_t unsigned int
	if (*checkaddr != vaddr) {
...
//get tracefile and access memory for each vaddr
void replay_trace(FILE *infp) {
	char buf[MAXLINE];
	addr_t vaddr = 0;
	char type;
//each line of tracefile contains type+virtual address 
	while(fgets(buf, MAXLINE, infp) != NULL) {
		if(buf[0] != '=') {
			sscanf(buf, "%c %lx", &type, &vaddr);
			if(debug)  {    //used for debug mode
				printf("%c %lx\n", type, vaddr);
			}
...
int main(int argc, char *argv[]) {
	...
	while ((opt = getopt(argc, argv, "f:m:a:s:")) != -1) {
		...
		case 'f':
			...
			break;
		case 'm':
			memsize = (unsigned)strtoul(optarg, NULL, 10);// base on 10[decimal,convert 'optarg' to an unsigned long integer]
			break;
		case 'a':
			replacement_alg = optarg;
			break;
		case 's':
			swapsize = (unsigned)strtoul(optarg, NULL, 10);//# of pages
			break;
		default:
	...
	if(replacement_alg == NULL) {
		...
		int i;
		//find the replacement algorithm we are gonna use
		for (i = 0; i < num_algs; i++) {   //num_algs=5
	...
	replay_trace(tfp);   //tfp=FILE *[tracefile]
	print_pagedirectory();
