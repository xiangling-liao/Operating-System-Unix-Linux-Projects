...
static int add_pid_sysc(pid_t pid, int sysc)
{
	...
	INIT_LIST_HEAD(&ple->list);//create a list node
	ple->pid=pid;

	list_add(&ple->list, &(table[sysc].my_list));//the second parameter is passed as the head of the list, the first one is the node we want to add
...
{
    spin_lock(&pidlist_lock);//have to lock before make change to the pid_list
    del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    if(table[sysc].monitored==2){
    	ismonitered=check_pid_monitored(sysc,current->pid);
    	if(ismonitered==1){
             return table[sysc].f(reg);//blacklist
    	}
 ...
 *      For the last two commands, the following logic applies:set_addr_r
...
asmlinkage long my_syscall_intercept_release(int syscall, int pid){
    spin_lock(&calltable_lock);
    set_addr_rw((unsigned long) sys_call_table);
...
if((((syscall<0)|(syscall>NR_syscalls))|(syscall==MY_CUSTOM_SYSCALL))) { //check the validaty of the syscall number
                return -EINVAL;
             }
    ...
    switch(cmd){
        case REQUEST_SYSCALL_INTERCEPT:
            ...
            if(table[syscall].intercepted==1)
                return -EBUSY;
            my_syscall_intercept(syscall,pid);
            break;
        ...
        case REQUEST_SYSCALL_RELEASE:
            ...
            if(table[syscall].intercepted==0)
                return -EINVAL;
             my_syscall_intercept_release(syscall, pid);
            break;

        case REQUEST_START_MONITORING:
        if(table[syscall].intercepted==0) {
            	// printk(KERN_ALERT "Not being intercepted\n");
                return -EINVAL;
            }
            if(pid==0){
                if(current_uid()!=0){
                    ...
                    printk(KERN_ALERT "enter st\n");
                    //my_syscall_start_monitor(syscall, pid);

                 }
                               return 0;
           } else{
                    int isowner=check_pid_from_list(pid, current->pid); //  It is important to check if the calling process is the owner of the process which should be monitered by the system call
                    if(isowner!=0&&current_uid()!=0){
                        return -EPERM;
                    }

                    if(table[syscall].monitored==2){
                    	if(check_pid_monitored(syscall, pid)==1){//if all pid is monitored, then the list is a black list storing all pid should not be monitored.
                      del_pid_sysc(pid, syscall);
                    }else{
                       return -EBUSY;
                    }
        ...
        case REQUEST_STOP_MONITORING:
            ...
            if(table[syscall].intercepted==0) {
             ...
             return 0;
          	...
          	int isowner=check_pid_from_list(pid, current->pid);
            if(isowner!=0&&current_uid()!=0){
                return -EPERM;
                }
          	if(table[syscall].monitored==2){//if all pic are monitored, but now you just want to a specific one
          	//	printk(KERN_ALERT "enter stop\n");
...
static int init_function(void) {
    ...
    spin_lock(&pidlist_lock);    //initializations for bookkeeping data structures. Set lock before make change to the monitored pid lists

    for(i=0; i<=NR_syscalls; i++){
        INIT_LIST_HEAD(&(table[i].my_list));
    }//no need initialize here, cuz in helper function, we will do that
